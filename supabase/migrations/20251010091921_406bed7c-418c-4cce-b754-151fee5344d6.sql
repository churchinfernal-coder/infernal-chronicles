...
-- Add blocked users table if it doesn't exist
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE table_name = 'profiles' AND column_name = 'allow_contact_requests'
  ) THEN
    ALTER TABLE profiles ADD COLUMN allow_contact_requests boolean DEFAULT true;
  END IF;
END $$;
